  ...
  Future<void> _addProduct() async {
    if (_formKey.currentState!.validate() && _image != null) {
      final request = http.MultipartRequest(
        'POST',
        Uri.parse(
            'https://api-delivery-application.vercel.app/add/products'), // เปลี่ยน URL เป็น API ของคุณ
      );
      ...
      request.files.add(await http.MultipartFile.fromPath(
        ...
        _image!.path,
        contentType:
            MediaType('image', 'jpeg'), // เปลี่ยนตามประเภทไฟล์ที่คุณอัปโหลด
      ));
      ...
      try {
        ...
        if (response.statusCode == 201) {
          ...
          final responseString =
              utf8.decode(responseData); // แปลงข้อมูลที่ได้รับเป็นข้อความ
